...
- **command 註冊是一個迴圈,不是清單。** `ging-git-view.branches.*` 的 17 條 command 由遍歷 catalogue 產生(`batch: true` 者加註冊 `<action>Selected`,`runsIn: "host"` 者接 host-handler 表)。grep command ID 只會命中 `package.json` —— 這是刻意的:手寫清單「漏一條」正是要消滅的那類靜默失配,catalogue 有就必然註冊。
