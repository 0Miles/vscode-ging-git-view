...
- **鍵盤導覽也得自己做,而且只做到選單內部。** 方向鍵、Home/End、Enter、Tab、`role=menu`/`menuitem`/`menuitemcheckbox` 都在 `showContextMenu` 附近手寫。但 commit 列、參照標籤、檔案列**仍不可 focus**,所以選單只能用滑鼠右鍵開啟,Shift+F10 沒有目標,螢幕閱讀器使用者到不了這些動作。讓來源元素可 focus 會逼著上下鍵從「捲動畫面」改成「在列之間移動」,那是獨立的一件事 —— 追蹤於 [#13](https://github.com/0Miles/vscode-ging-git-view/issues/13)。
