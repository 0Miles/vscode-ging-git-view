...
VS Code 從 TreeView 回報的原始選取集**含** folder 與 group 標題,因此不等於 branch selection:只反白資料夾時,原始集合非空而 branch selection 是空的。兩者之間的轉換由 `branchSelectionOf` 負責,問「有沒有選到分支」一律以它的結果為準,不要拿原始集合的大小當答案。
